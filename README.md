# Resonite Translator (JP <-> EN, WebGPU)

ブラウザの Web Speech API で日本語/英語を聞き取り、`onnx-community/LFM2-350M-ENJP-MT-ONNX` を **WebGPU** でローカル実行して翻訳し、結果を Resonite ユーザー名ごとの WebSocket チャンネルに配信します。
...
Browser (Chrome, WebGPU)            Node server                    Resonite
  ...
  Transformers.js   ─┼──▶  Map<user, Set<WS>>  ──── ws://host:8080/<name> ─▶ WS Client
                     │
                     └── translate locally
```
...
2. 初回はモデルがダウンロードされる（数百 MB、進捗が表示される）
